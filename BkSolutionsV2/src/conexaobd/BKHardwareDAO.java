...
public class BKHardwareDAO {
	...
	public synchronized Hardware getHardware(String mac) throws SQLException, ClassNotFoundException {
		Hardware hardware = null;
		BKModeloDAO modeloDAO = new BKModeloDAO();
		PreparedStatement ps = null;
		Connection conexao = MysqlConnection.getConnection();
		ps = conexao.prepareStatement("SELECT * FROM bk_solutions.hardware WHERE hardware_mac = ?");
		ps.setString(1, mac);
		ResultSet rs = null;
		rs = ps.executeQuery();
		if (rs.first()) {
